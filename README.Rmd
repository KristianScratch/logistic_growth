...
The goal of the analysis is to take a single sample of an _Escherichia coli_ isolate growing in a growth media and estimate key growth paramiters.  Namely, The starting population (No), the intrinsic growth rate (r) which is the exponential phase before density dependent limiting, and the carrying capacity (k). For reference,  I used experiment 2's csv datafile.

So,  we first Interrogated the data and plotted it ti see the sigmoidal curve, plotting the count "N" (y) against time (x). But this is hard to extract the useful parameters from. So, we can use log(N) as the response instead, this transformation allows us to more easily get the r value later. All of these initial plotting code can be found in "plot_data.R".
...
Finding 'r': 
We can find r from this transformed graph by measuring the gradient when far from carrying capacity,  aka when 't' is low. For this, I chose t is less than 240 minutes to be safe -it seems to plateu at around 500.So, we subsetted the data to only <240, and analyised this with lm() as this is now eefectively linear, the gradient (r) was *0.0303057*.

Finding 'No:
From the same linear model we can see that the starting pop is shown as the intercept. But remebering it is in ln(No), so No is e^7.5584677. AkA we estimate the starting pop (No) as *1917* cells. 
...
We can use these parameters to manually set out custom logistic model function and produce a model for the growth data. This code is shown in "plot_data_and_model.R"

##Question 2

IF we assume the populaiton grows exponentially indefientely, then we assume the r of 0.0303057 remians. 
So N = No * (exp(r*t)). 
...
The code for comparing the Two models (exponential and logistic) can be found in "Comparing_Models.R". We can also call it here:

```{r}
print(Comparing_Models_plot)
```

![Comparing_Models_PLOT](https://github.com/KristianScratch/logistic_growth/assets/150162503/45315f55-63a9-477d-8b40-58e6004199b3)
